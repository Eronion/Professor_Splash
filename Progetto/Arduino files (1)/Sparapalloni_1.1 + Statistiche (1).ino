...
void loop() {
    ...
    if (iState==1 && iReceived==0) iState=5; //Off
    //if (iState==2 && iReceived==0) iState=5; //Forse dovrei permettere di spegnere la macchina solo nello stato 
    //if (iState==3 && iReceived==0) iState=5; // 1 per evitare di lasciare la macchina in uno stato inconsistente
    //if (iState==4 && iReceived==0) iState=5; 
	
	if (iReceived==9) iState=0; //Posso poi aggiungere un altro tasto per bloccarla qualunque sia lo stato
    ...
    //per statistiche potrei anche inviare al bridge ad ogni ciclo il numero di tirati e realizzati
	// così da visionare in tempo reale le cose nell'app. Ciò toglie un po' la necessita dello Stato 5.
	//potrei controllare se c'è stato un incremento dei tirati o realizzati con un if che entra se il valore
	// è più grande del vecchio e il vecchio lo metto uguale al nuovo ogni volta che entro.
	// l'unica cosa è capire come gestire lato bridge il tutto
