...
void loop() {
  ...
  if (Serial.available()>0) iReceived = Serial.read() - 48; //perchè se mando uno la seriale lo converte in ascii che è 49, 
    ...
    if (iState==0 && iReceived==1) iFutureState=1;
    if (iState==1 && iReceived==0) iFutureState=5;
    if (iState==2 && iReceived==0) iFutureState=5;
    if (iState==3 && iReceived==0) iFutureState=5;
    if (iState==4 && iReceived==0) iFutureState=5;
    if (iState==1 && iImposed==2){
		 iFutureState=2;
		 time2=millis();
		}
    if (iState==2 && iImposed==3) iFutureState=3;
    if (iState==3 && iImposed==4) iFutureState=4;
    if (iState==4 && iImposed==1) iFutureState=1;
    if (iState==5 && iImposed==0) iFutureState=0;
    ...
    if (iReceived==10 || iReceived==13) iFutureState=iState;
     ...
     if (iFutureState==1 && iState==0){
		 //controllo se ci sono palle disponibili
		 Serial.println("- Entrato in stato 1 -");
     Serial.println(distance);
		 if (distance < 1 || distance >50) {  // This is where the LED On/Off happens     digitalWrite(led,HIGH); // When the Red condition is met, the Green LED should turn off   digitalWrite(led2,LOW); }   else {     digitalWrite(led,LOW);     digitalWrite(led2,HIGH);   }   if (distance >= 200 || distance <= 0){
     ...
     if ( iFutureState==2 && iState==1){
		 ...
		 //dispenser.write(90); Non credo serva
		 //if (millis() - time2 > 100){ //vedi un un po' poi dalla pratica che valore dare  se 600 o più
		 dispenser.write(180);
		 //time3=millis();
		 //}
		 if (millis() - time2 > 500){//vedi un un po' poi dalla pratica che valore mettere dopo il >
			 dispenser.write(90);
	 ...
	 if (iFutureState==5 && iState==4){
		 ...
		 iFutureState=0;//alla fine ritorno allo stato di idle
	 }
