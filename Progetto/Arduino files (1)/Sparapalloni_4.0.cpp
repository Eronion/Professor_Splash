...
//Sensori di distanza
#define trigPin1 23 //Il sensore di distanza 1 vede se c'è una palla disponibile da far entrare nella macchina 
#define echoPin1 22
#define trigPin2 25 //Il sensore 2 cerca di capire quando un pallone entra nel canestro
#define echoPin2 24
#define trigPin3 27 //Il 3 misura la distanza del tiratore per capire se il tiro è da considerare da 2 o 3pt.
#define echoPin3 26

//Definiamo qui i pin di ingresso
// Servono tre servo motori, due sensori per rilevare palla (prossimità o distanza), dei led per gli 
//  identificare gli stati ed in più un ulteriore sensore di distanza che vede se il tiro e da 2 o 3pt, esso
//    mima quella che sarà nel progetto reale una telecamera che riconosce la persona e ne stima la distanza.

//Definisco dove collego i led
#define led_pin_StatoIdle  38
#define led_pin_StatoOn  39


//Pin bottoni  --> Ho eliminato i bottoni in favore di un sensore di distanza 
...
unsigned long time_ricarica; 
unsigned long time_invio_dati; 
long duration_check, distance_check; //variabili controllo presenza palla 
long duration_realizzati, distance_realizzati; //variabili controlla se palla entra nel canestro
long duration_posizioneTiro, distance_posizioneTiro;
//int Stato_bottone_2punti;
//int Stato_bottone_3punti;
int tirati_2punti;
int tirati_3punti;
int realizzati_2punti;
...
void setup() {
  ...
  duration_posizioneTiro=2000;
  distance_posizioneTiro=60;

  time_invio_dati=millis();
...
void loop() {
  ...
  if (millis() - time_loop > 200){ // read every 50 millisecond
  time_loop = millis();
  ...
  delayMicroseconds(10); 
  digitalWrite(trigPin3, LOW);
  duration_posizioneTiro = pulseIn(echoPin3, HIGH);
  distance_posizioneTiro = (duration_posizioneTiro/2) / 29.1;
  ...
  if(iState==0){ //magari fai due led separati uno rosso che dice quando è in idle e uno verde per quando è in funzione
    ...
    if (distance_check >20) {  // This is where the LED On/Off happens     digitalWrite(led,HIGH); // When the Red condition is met, the Green LED should turn off   digitalWrite(led2,LOW); }   else {     digitalWrite(led,LOW);     digitalWrite(led2,HIGH);   }   if (distance >= 200 || distance <= 0){
    ...
    if (iState==2){
     ...
     if ((millis() - time_check) < 1000){ //vedi un un po' poi dalla pratica che valore mettere dopo il >
       dispenser.write(180);
      } else {
       dispenser.write(90);
    ...
    if (iState==4){
     ...
     grilletto.write(180);
     //conto tirati
     if(distance_posizioneTiro<75){
      tirati_2punti=tirati_2punti+1;
     } else{
      tirati_3punti=tirati_3punti+1;
     }
     //attendo ora qualche secondo in cui ricarico la macchina e poi faccio partire la prossima palla
     if ((millis() - time_ricarica) > 500 && (millis() - time_ricarica) < 1000  ){
      //Serial.println("Ricarico");
      ricaricatore.write(1);
      } else if ((millis() - time_ricarica) > 1000){
      //Serial.println("Riinzializzo e impongo1");
      tenditore.write(1);
      grilletto.write(90);
      ricaricatore.write(90);
      iImposed=1;
  ...
  //dopo che passo ultimo passaggio torno in zero aspetto qualche secondo il canestro e poi azzero le stat
    ...
    if(distance_realizzati < 15){
      if(distance_posizioneTiro<75){
        realizzati_2punti=realizzati_2punti+1;
      }else{
        realizzati_3punti=realizzati_3punti+1;
      }
    }
     if (millis() - time_invio_dati > 5000){ // Invia dati ogni 5 secondi per evitare superamento rate
        time_invio_dati = millis();
        if(distance_posizioneTiro<75){
